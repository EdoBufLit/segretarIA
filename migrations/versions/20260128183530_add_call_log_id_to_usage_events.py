...
def upgrade():
    ...
    with op.batch_alter_table("usage_events") as batch:
        ...
        batch.alter_column("call_id", existing_type=sa.String(), nullable=False)

    if bind.dialect.name == "sqlite":
        op.execute(
            """
            UPDATE usage_events
            SET call_log_id = CAST(call_id AS INTEGER)
            WHERE call_id GLOB '[0-9]*'
...
def downgrade():
    with op.batch_alter_table("usage_events") as batch:
        batch.alter_column("call_id", existing_type=sa.String(), nullable=True)
